    ...
    <nav class="md:hidden fixed bottom-0 left-0 right-0 bg-gh-dark/95 backdrop-blur-xl border-t border-gh-border/60 z-50 safe-area-bottom">
      <div class="flex justify-around items-center h-16 px-2">
        <router-link
          ...
          :key="item.to"
          :to="item.to"
          class="flex flex-col items-center gap-0.5 px-1 py-1.5 rounded-xl text-[9px] font-medium transition-all duration-300 min-w-[44px]"
          :class="isActive(item.to)
            ? 'text-gh-primary'
            : 'text-gray-600 hover:text-gray-300 active:text-gray-400'"
        >
