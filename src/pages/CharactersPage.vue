...
function handleDelete(charUuid: string) {
  characterStore.deleteCharacter(charUuid)
      ...
      >
        ...
        <div v-if="selectedUuid === char.uuid" class="border-t border-gh-border/50 p-5">
          ...
          <div class="pt-4 border-t border-gh-border/30 space-y-3">
            ...
            <!-- Delete confirmation -->
            <div v-if="showDeleteConfirm === char.uuid" class="bg-red-900/10 rounded-xl border border-red-800/20 p-4">
              <div class="flex items-center gap-2 mb-3">
                ...
                <span class="text-sm font-semibold text-red-300">Odstranit {{ char.playerName }}?</span>
              </div>

              <p class="text-xs text-gray-400 mb-3 leading-relaxed">
                Postava bude trvale smazána. Tato akce je nevratná a postava nebude v archivu.
              </p>

              <div class="flex gap-2">
                <button
                  class="py-2 px-4 bg-red-600 text-white rounded-lg hover:bg-red-500 transition-colors text-xs font-medium"
                  @click="handleDelete(char.uuid)"
                >
                  Smazat postavu
                </button>
                <button
