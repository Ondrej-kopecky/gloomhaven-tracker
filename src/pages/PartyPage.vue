...
<template>
  <div v-if="campaignStore.hasCampaign && partyStore.party" class="max-w-4xl mx-auto">
    ...
    <div class="grid md:grid-cols-2 gap-4">
      ...
      <!-- Scenario level -->
      <div class="gh-card relative overflow-hidden p-6 flex flex-col">
        <div class="absolute left-0 top-0 bottom-0 w-[3px] bg-orange-500"></div>
        <div class="flex items-center gap-2 mb-4">
          ...
          <h3 class="text-[11px] font-semibold text-gray-500 uppercase tracking-wider">Úroveň scénáře</h3>
        </div>

        <div class="text-center flex-1 flex flex-col justify-center">
          <div>
            <span class="font-display text-4xl sm:text-5xl font-bold text-orange-400">{{ scenarioLevel }}</span>
            <span class="text-gray-500 text-sm ml-1">doporučená</span>
          </div>

          <!-- level scale 1-7 -->
          <div class="flex items-center justify-center gap-1.5 mt-3 mb-3">
            <div
              v-for="i in 7"
              :key="i"
              class="w-3 h-3 rounded-full transition-all"
              :class="i <= scenarioLevel ? 'bg-orange-500' : 'bg-white/[0.06]'"
            ></div>
          </div>
          <div class="text-[10px] text-gray-600 leading-relaxed">
            z průměru úrovní postav ({{ characterStore.averageLevel }}) ÷ 2 — u stolu lze upravit nahoru i dolů
          </div>
        </div>
      </div>
    </div>
